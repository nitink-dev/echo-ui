...
export function ScannerTable({
  ...
  const initialConnected = useMemo(() => {
    ...
    scanners.forEach(s => { map[s.deviceSerialNumber] = (s as any).connected ?? false; });
    return map;
  }, [scanners]);

  const initialResearch = useMemo(() => {
    const map: Record<string, boolean> = {};
    scanners.forEach(s => { map[s.deviceSerialNumber] = (s as any).research ?? false; });
    return map;
  }, [scanners]);

  const [connectedMap, setConnectedMap] = useState<Record<string, boolean>>(initialConnected);
  const [researchMap, setResearchMap] = useState<Record<string, boolean>>(initialResearch);
      ...
      <Table className="w-full">
        <TableHeader>
          <TableRow className="hover:bg-transparent border-gray-200 bg-gray-50">
            <TableHead className="font-semibold text-gray-700 px-4 py-3 w-[26%]">Scanner Details</TableHead>
            <TableHead className="font-semibold text-gray-700 px-4 py-3 w-[14%]">AE Title</TableHead>
            <TableHead className="font-semibold text-gray-700 px-4 py-3 w-[18%]">Model &amp; Serial</TableHead>
            <TableHead className="font-semibold text-gray-700 px-4 py-3 w-[20%]">Location</TableHead>
            {/* Research (visible header) */}
            <TableHead className="font-semibold text-gray-700 px-4 py-3 w-[12%]">Research</TableHead>
        ...
        <TableBody>
          {scanners.map((scanner, index) => {
            ...
            const connected = connectedMap[key] ?? (scanner as any).connected ?? false;
            const research = researchMap[key] ?? (scanner as any).research ?? false;
            const researchDisabled = true;//!connected;
            const dispatchUpdate = async (partial: Partial<SlideScanner>) => {
              try {
                const payload: SlideScanner = { ...scanner, ...partial } as SlideScanner;
                ...
                <TableCell className="px-4 py-3" onClick={() => onViewScanner(scanner)}>
                  <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                    <Checkbox
                      ...
                      aria-label="Research"
                    />
                    <span className="text-xs text-gray-600">{research ? 'Yes' : 'No'}</span>
                  </div>
                </TableCell>


                <TableCell className="px-4 py-3 " width="120px" onClick={() => onViewScanner(scanner)}>
                  <div
                  ...
                  >
                    ...
                    <span
                      className={`mt-1 text-sm ${connected ? "text-green-600" : "text-red-600"
                        }`}
                    >
