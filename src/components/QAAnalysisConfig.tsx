...
export function QAAnalysisConfig({ }: QAAnalysisConfigProps) {
  ...
  const [dicomStoreAddress, setDicomStoreAddress] = useState();
  ...
  const handleSaveParameter = async () => {
    ...
    const payload = { "barcode": parameterFormData.barcode, "activationCode": parameterFormData.activationCode }
    try {
      if (editingParameter) {
  ...
  const handleSaveDicom = async () => {
    ...
    let payload = {
      "gcp-config.pathqa-store-url": dicomStoreAddress.trim()
    }

    await dispatch(updateDicomStore(dicomStoreAddress));

    //setDicomStoreAddress(dicomStoreAddress);
                  ...
                  <TableRow key={parameter.id}>
                    ...
                    <TableCell className="font-mono relative flex items-center gap-2">
                      ...
                      >
                        ...
                        {visibleActivationCodes[parameter.id] ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
