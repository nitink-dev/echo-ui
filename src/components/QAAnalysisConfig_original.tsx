...
export function QAAnalysisConfig({}: QAAnalysisConfigProps) {
  const BASE_URL="http://10.201.8.208:8080/api";
...
const handleDeleteConfirm = async () => {
  ...
  const handleSaveDicom = async() => {
    ...
    let payload = {
    "path.qa-dicon-store": tempDicomAddress.trim()
    }

  try{
    const res = await axios.patch(`${BASE_URL}/config/path-qa/dicom-store`, payload)
    .then(result =>console.log("RESULT: "+result.data))
    .catch(error => console.log("RESULT: "+error))
    } catch (error) {
        console.error("Error editing  dicom url:", error);
      }
