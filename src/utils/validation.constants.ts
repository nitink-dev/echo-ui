...
export const isValidIP = (v: string): boolean =>
  IPV4_REGEX.test(v.trim()) || IPV6_REGEX.test(v.trim());
...
export const isValidPort = (v: string): boolean =>
  PORT_VALID_PATTERN.test(v.trim());
...
export interface FieldRule {
  ...
  validate?: (v: string) => boolean; // custom validator (overrides validPattern)
  errorMessage: string;
  required?: boolean;
}

// ── Helper: sanitize a string to only allowed chars ───────────────────────────
export const sanitizeByPattern = (value: string, pattern: RegExp): string =>
  value
    .split("")
