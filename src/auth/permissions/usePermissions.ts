...
export function usePermissions() {
  ...
  if (!engine) {
    ...
    return {
      ...
      canGet: (api?: string) => {
        console.warn(`[usePermissions] canGet("${api}") → false (no engine)`);
        return false;
      },
      canPost: (api?: string) => {
        console.warn(`[usePermissions] canPost("${api}") → false (no engine)`);
        return false;
      },
      canPut: (api?: string) => {
        console.warn(`[usePermissions] canPut("${api}") → false (no engine)`);
        return false;
      },
      canPatch: (api?: string) => {
        console.warn(`[usePermissions] canPatch("${api}") → false (no engine)`);
        return false;
      },
      canDelete: (api?: string) => {
        console.warn(`[usePermissions] canDelete("${api}") → false (no engine)`);
        return false;
      },
      canUpdate: (api?: string) => {
        console.warn(`[usePermissions] canUpdate("${api}") → false (no engine)`);
        return false;
      },
    };
  }
